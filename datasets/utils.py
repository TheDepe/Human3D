...
def voxelize(
    ...
    if "labels" in input_dict:
        ...
        if len(list_labels[0].shape) == 1:
            ...
            if clip_feature[0] is not None:
                # TODO can potentially go into data loader
                sampled_idx = [
                    np.random.choice(
                        clip_feat.shape[0],
                        num_queries,
                        replace=clip_feat.shape[0] < num_queries,
                    )
                    for clip_feat in clip_feature
                ]  # with replacement in case not enough clip features

                sampled_clip_feats = np.stack(
                    [
                        clip_feat[sampled_id, :]
                        for (clip_feat, sampled_id) in zip(
                            clip_feature, sampled_idx
                        )
                    ],
                    axis=0,
                )
                sampled_clip_pos = np.stack(
                    [
                        clip_pos[sampled_id, :]
                        for (clip_pos, sampled_id) in zip(
                            clip_center_point, sampled_idx
                        )
                    ],
                    axis=0,
                )

                sampled_clip_feats = torch.from_numpy(sampled_clip_feats)
                sampled_clip_pos = torch.from_numpy(sampled_clip_pos)
            else:
                sampled_clip_feats = None
                sampled_clip_pos = None
    else:
        target = []
        target_full = []
        sampled_clip_feats = None
...
def get_instance_masks(
    ...
    for batch_id in range(len(list_labels)):
        ...
        # sometimes data augmentation kills a complete human instance
        # map back to [1, ...] range such that hungarian matching does not return nans
        full_ids = (
            torch.div(full_ids, 1000, rounding_mode="trunc") * 1000
            + np.unique(full_ids % 1000, return_inverse=True)[1]
            + 1
        )

        human_mask = []
        for human_id in np.unique(full_ids % 1000):
            mask_idx = human_id == (full_ids % 1000)
            human_mask.append(masks[mask_idx].any(dim=0))

        human_mask = torch.stack(human_mask)
        human_labels = torch.zeros(human_mask.shape[0], dtype=torch.int64)
