...
from random import choice, random, randrange, sample, uniform
...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(
        self,
        dataset_name="humanseg",
        data_dir: Optional[Union[str, Tuple[str]]] = "data/processed/humanseg",
        label_db_filepath: Optional[
            str
        ] = "configs/humanseg_preprocessing/label_database.yaml",
        # mean std values
        ...
        self.chiral_swap = {
            class_id: self.chiral_classes[
                class_name.replace("right", "left")
                if "right" in class_name
                else class_name.replace("left", "right")
            ]
                    ...
                    if random() < 0.5:
                        ...
                        if not self.broken_mirror_version:
                            for point_id in range(labels.shape[0]):
                                ...
                                labels[point_id, 0] = self.chiral_swap[
                                    labels[point_id, 0]
                                ]
