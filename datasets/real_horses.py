...
class RealHorsesMPI_BU(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        mesh = trimesh.load(pcd_file, process=False) # Apply rotations
        mesh = apply_rotations(mesh, self.rotations) # Center the points
...
class RealHorsesMPI(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # Dummy features (intensity/color)
        features = np.ones_like(points)

        return {
            "points": points,       # [N, 3]
            "features": features,   # [N, 3] or whatever extra dims you want
            "path": str(pcd_file),
            "original_mesh": mesh
        }

def collate_sparse_batch_inference(batch, voxel_size=0.02, device=torch.device("cuda")):
    """
    Collate function for inference with MinkowskiEngine.
    Performs voxelization, quantization, sparse collate, and moves to GPU.
    """
    ...
    for item in batch:
        ...
        # Combine points and features if needed
        features_combined = np.hstack((feats, points))  # optional

        # Quantize
        _, _, unique_map, inverse_map = ME.utils.sparse_quantize(
            coordinates=torch.from_numpy(coords).contiguous(),
            features=torch.from_numpy(features_combined).float(),
            return_index=True,
            return_inverse=True,
        )

        # Keep only unique points
        coords_quant = coords[unique_map]
        feats_quant = features_combined[unique_map]

        coordinates_list.append(torch.from_numpy(coords_quant).int())
        features_list.append(torch.from_numpy(feats_quant[:, :3]).float().to(device))
        og_features_list.append(torch.from_numpy(feats_quant).float())
        meshes.append(item["original_mesh"])
        paths.append(item["path"])
        inverse_maps.append(inverse_map)

    # Sparse collate handles batching automatically
