...
import benchmark.util_3d_human_instance as util_3d
...
def make_pred_info(pred: dict):
    pred_info = {}
    assert (
        pred["pred_human_classes"].shape[0]
        == pred["pred_human_scores"].shape[0]
        == pred["pred_human_masks"].shape[1]
    )
    for i in range(len(pred["pred_human_classes"])):
        info = {}
        info["label_id"] = pred["pred_human_classes"][i]
        info["conf"] = pred["pred_human_scores"][i]
        info["mask"] = pred["pred_human_masks"][:, i]
...
def evaluate(preds: dict, gt_path: str):
    matches = {}
